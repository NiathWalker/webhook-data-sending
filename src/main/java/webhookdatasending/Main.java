    ...
    public void init() {
            ...
            if (!e.message.startsWith("/") ) {
                if (configValues.get("webhook-message-sending-toggle").equals("true")) {
                    if (configValues.get("webhook-admin-detection-toggle").equals("true")) {
                        if (e.player.admin) {
                            webhook(String.format("[chat] [admin] [%s]: %s", e.player.plainName(), e.message));
                        } else {
                            webhook(String.format("[chat] [player] [%s]: %s", e.player.plainName(), e.message));
                        }
                    } else {
                        webhook(String.format("[chat] [%s]: %s", e.player.plainName(), e.message));
                    }
                }
            }
        });
        ...
        Events.on(EventType.PlayerJoin.class, e -> {
            if (configValues.get("webhook-connection-sending-toggle").equals("true")) {
                if (configValues.get("webhook-admin-detection-toggle").equals("true")) {
                    if (e.player.admin) {
                        webhook(String.format("[join] [admin] [%s]", e.player.plainName()));
                    } else {
                        webhook(String.format("[join] [player] [%s]", e.player.plainName()));
                    }
                } else {
                    webhook(String.format("[join] [%s]", e.player.plainName()));
                }
            }
        });
        ...
        Events.on(EventType.PlayerLeave.class, e -> {
            if (configValues.get("webhook-disconnection-sending-toggle").equals("true")) {
                if (configValues.get("webhook-admin-detection-toggle").equals("true")) {
                    if (e.player.admin) {
                        webhook(String.format("[leave] [admin] [%s]", e.player.plainName()));
                    } else {
                        webhook(String.format("[leave] [player] [%s]", e.player.plainName()));
                    }
                } else {
                    webhook(String.format("[leave] [%s]", e.player.plainName()));
                }
            }
        });
    }
